  ...
  <div class="relative w-full" ref="outerElment">
    <div class="none-scrollbar flex w-full max-w-full flex-row space-x-10 overflow-x-auto" ref="scrollElement">
      <Tag
        ...
        :disabled="(itemActive && !item[`${itemActive}`]) || false"
    ...
    <div v-if="isDesktop">
      <div
        v-show="showLeft"
        class="absolute left-0 top-0 flex h-full w-24 transform-gpu cursor-pointer items-center justify-center rounded-r-xl bg-black bg-opacity-40 backdrop-blur-sm"
        @click="moveToLeft"
      >
      ...
      <div
        v-show="showRight"
        class="absolute right-0 top-0 flex h-full w-24 transform-gpu cursor-pointer items-center justify-center rounded-l-xl bg-black bg-opacity-40 backdrop-blur-sm"
        @click="moveToRight"
      >
